...
app.config['UPLAOD_FOLDER']=str(cwd)+"\static\images"
app.config["FILE_UPLOADS"] = str(cwd)+"\static\images"
...
def get_SD_log():
    ...
    if request.method == 'POST':
        ...
        if SD_log_address:
            ...
            temp.to_csv(str(cwd)+"\static\images\SDLog2ShowInside.csv", index=False)
...
def get_SD_Resutl():
    corr_df = pd.read_csv(str(cwd)+"\static\images\SDLog2ShowInside.csv")
...
def get_mapping():
    ...
    data = pd.read_csv(str(cwd)+"\static\images\SDLog2ShowInside.csv")
